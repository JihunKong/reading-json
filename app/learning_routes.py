...
• 글 전체를 통해 작가는 [주제의식]을 전달하고자 한다."
        ...
        function submitCurrentPhase() {
            console.log('🔥 submitCurrentPhase 호출됨, currentPhase:', currentPhase);
            ...
            if (currentPhase === 1) {
                ...
                console.log('📊 Phase 1 데이터 수집:', responseData);
            ...
            console.log('📤 서버로 전송할 데이터:', responseData);
            ...
            .then(data => {
                ...
                if (data.success) {
                    console.log('✅ 서버 응답 성공:', data.evaluation);
        ...
        function showPhaseResult(evaluation) {
            console.log('🎯 showPhaseResult 시작:', evaluation);
            ...
            console.log('📊 평가 결과 상세:', {
                score: evaluation.score,
                mastery_achieved: evaluation.mastery_achieved,
                currentPhase: currentPhase,
                scorePercent: (evaluation.score * 100).toFixed(1)
            });
            
            if (evaluation.mastery_achieved) {
                console.log('✅ 숙련도 달성됨 - 다음 단계 활성화');
                ...
                console.log('✅ Phase 완료 표시:', phaseCompleted);
                ...
                if (nextBtn) {
                    ...
                    console.log('🎯 다음 단계 버튼 표시됨');
                } else {
                    console.error('❌ nextBtn 요소를 찾을 수 없음');
                }
                
                // Remove celebration overlay if it exists
                const celebration = document.getElementById('celebrationOverlay');
                if (celebration) {
                    celebration.remove();
                    console.log('🎉 축하 모달 제거됨');
                }
        ...
        function setupDragEvents() {
            ...
            draggables.forEach(element => {
                ...
                // Touch events for mobile (check if functions exist)
        ...
        function collectPhase1Data() {
            ...
            console.log('🔍 수집할 데이터:', {
                ...
                selectedCount: Object.keys(selectedComponents).length
            });
            ...
            const collectedData = {
                ...
                identified_components: selectedComponents, // This is what the backend expects
            ...
            console.log('📤 최종 수집 데이터:', collectedData);
        ...
        function initializePhase1Interaction(data) {
            ...
            console.log('🎯 찾을 성분들:', componentsToFind);
            
            // Store data globally for collection
            window.currentPhase1Data = {
                targetSentence: data.target_sentence,
                selectedComponents: selectedComponents,
                componentsToFind: componentsToFind,
                startTime: Date.now(),
                hintsUsed: 0
            };
            
            console.log('🌍 전역 데이터 저장됨:', window.currentPhase1Data);
            
            // Create phrase-clickable interface using phrase analysis data
            const phraseAnalysis = data.target_sentence.phrase_analysis || [];
            console.log('🔍 Phrase analysis data:', phraseAnalysis);
            
            let clickableHTML = sentenceText;
            let phraseIndex = 0;
            
            // If we have phrase analysis data, use it
            if (phraseAnalysis.length > 0) {
                // Sort phrases by start position to avoid overlaps
                const sortedPhrases = [...phraseAnalysis].sort((a, b) => a.start_pos - b.start_pos);
                
                // Build HTML with clickable phrases
                clickableHTML = '';
                let currentPos = 0;
                const originalWords = sentenceText.split(' ');
                
                // Simple approach: just make each phrase clickable
                sortedPhrases.forEach((phrase, index) => {
                    if (index > 0) clickableHTML += ' ';
                    
                    // Add the clickable phrase
                    clickableHTML += `<span class="clickable-phrase" 
                                           data-phrase-index="${index}" 
                                           data-original="${phrase.text}"
                                           data-component-type="${phrase.component_type}"
                                           data-confidence="${phrase.confidence}"
                                           title="구 단위 성분: ${phrase.component_type} (신뢰도: ${phrase.confidence.toFixed(2)})"
                                           style="background-color: #f0f0ff; padding: 2px 4px; margin: 1px; border-radius: 3px; cursor: pointer; border: 1px solid #ccc;">
                                        ${phrase.text}
                                      </span>`;
                });
            } else {
                // Fallback to word-level interface if no phrase analysis
                console.log('⚠️ No phrase analysis available, using word-level fallback');
                const words = sentenceText.split(/(\s+|[.,!?;:()"''])/);
                let wordIndex = 0;
                
                clickableHTML = words.map(word => {
            ...
            targetSentence.innerHTML = `
                <div class="sentence-container">
                    ...
                    <div class="identification-hints" id="identificationHints">
                        <p>💡 구(phrase)를 클릭하여 문장 성분을 식별해보세요 (조사 포함)</p>
                    </div>
                </div>
            `;
            
            // Enhanced click listeners for phrase-based interaction
            const clickablePhrases = document.querySelectorAll('.clickable-phrase');
            const clickableWords = document.querySelectorAll('.clickable-word');
            
            clickablePhrases.forEach(phrase => {
                phrase.addEventListener('click', handlePhraseClick);
                phrase.addEventListener('mouseover', showPhraseHint);
                phrase.addEventListener('mouseout', hidePhraseHint);
            });
            ...
            function deselectWord(wordElement, wordIndex) {
                ...
                delete window.currentPhase1Data.selectedComponents[wordIndex];
            ...
            // Phrase-level click handling functions
            function handlePhraseClick(event) {
                const phraseText = this.getAttribute('data-original');
                const phraseIndex = this.getAttribute('data-phrase-index');
                const suggestedComponent = this.getAttribute('data-component-type');
                
                if (this.classList.contains('selected')) {
                    // Deselect with confirmation
                    if (confirm(`"${phraseText}" 구의 선택을 취소하시겠습니까?`)) {
                        deselectPhrase(this, phraseIndex);
                    }
                } else {
                    // Show enhanced phrase component selection modal
                    showPhraseComponentSelectionModal(phraseText, phraseIndex, this, suggestedComponent);
                }
            }
            
            function deselectPhrase(phraseElement, phraseIndex) {
                phraseElement.classList.remove('selected');
                phraseElement.removeAttribute('data-component');
                phraseElement.style.background = '';
                delete window.currentPhase1Data.selectedComponents[phraseIndex];
                updatePhase1Progress();
                showFeedbackMessage(`"${phraseElement.getAttribute('data-original')}" 구 선택이 취소되었습니다`, 'info', 2000);
            }
            
            function showPhraseHint(event) {
                const phraseText = this.getAttribute('data-original');
                const componentType = this.getAttribute('data-component-type');
                const confidence = this.getAttribute('data-confidence');
                
                const tooltip = document.createElement('div');
                tooltip.className = 'phrase-tooltip';
                tooltip.innerHTML = `
                    <strong>${phraseText}</strong><br>
                    <small>제안 성분: ${componentType}</small><br>
                    <small>신뢰도: ${(parseFloat(confidence) * 100).toFixed(0)}%</small><br>
                    <em>클릭하여 성분 선택</em>
                `;
                document.body.appendChild(tooltip);
                        ...
                        <div class="component-modal">
                            ...
                            <div class="component-options">
                                ...
                                <div class="options-grid">
                                    ${window.currentPhase1Data.componentsToFind.map(comp => `
            ...
            // Phrase Component Selection Modal
            function showPhraseComponentSelectionModal(phraseText, phraseIndex, phraseElement, suggestedComponent) {
                const modalHTML = `
                    <div class="component-modal-overlay" id="componentModal">
                        <div class="component-modal">
                            <div class="modal-header">
                                <h4>"${phraseText}" 구의 문장 성분 식별</h4>
                                <button class="close-modal" onclick="closeComponentModal()">&times;</button>
                            </div>
                            
                            <div class="phrase-context">
                                <p><strong>전체 문장:</strong> ${data.target_sentence.text}</p>
                                <p><strong>선택된 구:</strong> <span class="highlight">${phraseText}</span></p>
                                <p><strong>AI 제안 성분:</strong> <span class="suggested-component">${suggestedComponent}</span></p>
                            </div>
                            
                            <div class="component-guide">
                                <h5>📚 구 단위 문장 성분 가이드</h5>
                                <div class="guide-grid">
                                    <div class="guide-item ${suggestedComponent === '주어구' ? 'suggested' : ''}">
                                        <strong>주어구:</strong> 누가/무엇이 (조사 포함)
                                    </div>
                                    <div class="guide-item ${suggestedComponent === '서술어구' ? 'suggested' : ''}">
                                        <strong>서술어구:</strong> 어떻게 하다/어떠하다 (완전한 서술)
                                    </div>
                                    <div class="guide-item ${suggestedComponent === '목적어구' ? 'suggested' : ''}">
                                        <strong>목적어구:</strong> 무엇을/누구를 (조사 포함)
                                    </div>
                                    <div class="guide-item ${suggestedComponent === '부사어구' ? 'suggested' : ''}">
                                        <strong>부사어구:</strong> 언제/어디서/어떻게 (다른 구 수식)
                                    </div>
                                    <div class="guide-item ${suggestedComponent === '보어구' ? 'suggested' : ''}">
                                        <strong>보어구:</strong> 어떻게/무엇으로 (주어 보충)
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="component-options">
                                ...
                                <div class="options-grid">
                                    ${window.currentPhase1Data.componentsToFind.map(comp => `
                                        <button class="component-btn ${comp} ${comp === suggestedComponent ? 'suggested-btn' : ''}" 
                                                onclick="selectPhraseComponent('${phraseIndex}', '${comp}', '${phraseText}')">
                                            <div class="btn-content">
                                                <div class="component-name">${getComponentDisplayName(comp)}</div>
                                                <div class="component-hint">${getPhraseComponentHint(comp, phraseText)}</div>
                                                ${comp === suggestedComponent ? '<div class="ai-suggestion">🤖 AI 추천</div>' : ''}
                                            </div>
                                        </button>
                                    `).join('')}
                                </div>
            ...
            function getPhraseComponentHint(comp, phraseText) {
                const hints = {
                    '주어구': `"${phraseText}"이(가) 동작의 주체 구인가요?`,
                    '서술어구': `"${phraseText}"이(가) 완전한 서술 구인가요?`,
                    '목적어구': `"${phraseText}"이(가) 동작의 대상 구인가요?`,
                    '보어구': `"${phraseText}"이(가) 주어를 보충하는 구인가요?`,
                    '부사어구': `"${phraseText}"이(가) 다른 성분을 수식하는 구인가요?`
                };
                return hints[comp] || '이 성분이 맞는지 생각해보세요';
            }
            
            window.selectPhraseComponent = function(phraseIndex, component, phraseText) {
                console.log(`구 성분 선택: ${phraseText} -> ${component}`);
                
                // Store the selection
                window.currentPhase1Data.selectedComponents[phraseIndex] = {
                    text: phraseText,
                    component: component
                };
                
                // Update visual feedback
                const phraseElement = document.querySelector(`[data-phrase-index="${phraseIndex}"]`);
                if (phraseElement) {
                    phraseElement.classList.add('selected');
                    phraseElement.setAttribute('data-component', component);
                    phraseElement.style.background = getComponentColor(component);
                    phraseElement.style.border = '2px solid #333';
                }
                ...
                // Show feedback
                showFeedbackMessage(`구 "${phraseText}"을(를) ${component}로 식별했습니다!`, 'success', 3000);
            }
            ...
            function getComponentColor(component) {
                ...
                return colors[component] || '#f0f0f0';
            }
            ...
            function showCompletionCelebration() {
                    ...
                    <div class="celebration-overlay" id="celebrationOverlay">
                        <div class="celebration-modal">
                            <div class="celebration-content">
                                <h2>🎉 훌륭합니다!</h2>
                                <p>Phase 1을 성공적으로 완료했습니다!</p>
                                <p>문장 성분을 잘 찾아내셨네요.</p>
                                <div class="celebration-actions">
                                    <button class="btn btn-success" onclick="submitCurrentPhase(); document.getElementById('celebrationOverlay').remove();">답안 제출</button>
                                    <button class="btn btn-secondary" onclick="document.getElementById('celebrationOverlay').remove();">계속 학습</button>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                // Show submit button when phase 1 is completed
                const actionButtons = document.getElementById('actionButtons');
                const submitBtn = document.getElementById('submitBtn');
                if (actionButtons && submitBtn) {
                    actionButtons.style.display = 'block';
                    submitBtn.style.display = 'block';
                    submitBtn.disabled = false;
                    
                    // Force add event listener to ensure it works
                ...
                // Add celebration overlay styles
            ...
            window.selectComponent = function(wordIndex, componentType, wordText) {
                console.log('🎯 성분 선택 시작:', {
                    wordIndex: wordIndex,
                    componentType: componentType,
                    wordText: wordText
                });
                
                // Validate window.currentPhase1Data exists
                if (!window.currentPhase1Data) {
                    console.error('❌ currentPhase1Data 초기화되지 않음');
                    return;
                }
                
                const wordElement = document.querySelector(`[data-word-index="${wordIndex}"]`);
                if (!wordElement) {
                    console.error('❌ Word element not found for index:', wordIndex);
                    return;
                }
                ...
                if (!window.currentPhase1Data.selectedComponents) {
                    window.currentPhase1Data.selectedComponents = {};
                    console.log('🔧 selectedComponents 객체 초기화됨');
                }
                
                window.currentPhase1Data.selectedComponents[wordIndex] = {
                    text: wordText,
                ...
                console.log('💾 성분 저장됨:', {
                    wordIndex: wordIndex,
                    stored: window.currentPhase1Data.selectedComponents[wordIndex],
                    totalSelected: Object.keys(window.currentPhase1Data.selectedComponents).length
                });
                
                // Show feedback
                showFeedbackMessage(`"${wordText}"를 ${componentType}로 분류했습니다.`, 'success');
                ...
                const selectedCount = Object.keys(window.currentPhase1Data.selectedComponents || {}).length;
                const targetCount = window.currentPhase1Data?.componentsToFind?.length || 3;
                const minRequired = Math.max(2, targetCount); // At least 2 components needed
                
                console.log('🔍 완료 상태 체크:', {
                    selectedCount: selectedCount,
                    targetCount: targetCount,
                    minRequired: minRequired,
                    shouldComplete: selectedCount >= minRequired
                });
                
                // Complete when we have enough components
                if (selectedCount >= minRequired) {
                    console.log('✅ Phase 1 완료 조건 달성!');
                    showCompletionCelebration();
                } else {
                    console.log(`⏳ 더 필요한 성분: ${minRequired - selectedCount}개`);
                }
            }
                    ...
                    <div class="celebration-overlay">
                        <div class="celebration-content">
                            ...
                            <div style="display: flex; gap: 10px; justify-content: center;">
                                <button class="btn" onclick="this.parentElement.parentElement.parentElement.remove()">닫기</button>
                                <button class="btn btn-success" onclick="submitCurrentPhase(); this.parentElement.parentElement.parentElement.remove();">답안 제출</button>
                            </div>
                        </div>
                    </div>
                `;
            ...
            // NOTE: selectComponent function is defined earlier in the file
            ...
            function updatePhase1Progress() {
                const totalToFind = Math.max(2, window.currentPhase1Data?.componentsToFind?.length || 3); // Minimum 2 components needed
                const foundComponents = Object.keys(window.currentPhase1Data?.selectedComponents || {}).length;
                const progressPercent = Math.min(100, (foundComponents / Math.max(2, totalToFind)) * 100); // Cap at 100%
                
                const statusMessage = document.getElementById('statusMessage');
                if (!statusMessage) return; // Exit if element not found
            ...
            // Data already stored in window.currentPhase1Data during initialization above
